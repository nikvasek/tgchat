...
class MonitorScheduler:
    ...
    async def run_global(self) -> int:
        await self.store.sync_from_google()
        app_config = await self.store.load()
        exporter = create_exporter(self.env, app_config)
        searcher = GlobalSearcher(self.client, app_config, self.env, exporter)
        return await searcher.search_once()

    async def run_cycle(self) -> tuple[int, int]:
        monitor_added = await self.run_monitor()
        global_added = await self.run_global()
        return monitor_added, global_added

    async def _loop(self) -> None:
        while self._running:
            app_config = await self.store.load()
            interval = app_config.monitor.poll_interval

            try:
                monitor_added, global_added = await self.run_cycle()
                if monitor_added or global_added:
                    app_config = await self.store.load()
                    ...
                    text = (
                        ...
                        f"Мониторинг: +{monitor_added}\n"
                        f"Глобальный поиск: +{global_added}"
                    )
                    if google_url:
                        text += f"\n\nGoogle Таблица:\n{google_url}"
                ...
                await self._notify_admins(
                    f"Ошибка проверки: {short_error}\n\n"
                    "Если бот не отвечает — напишите ему /start"
                )
